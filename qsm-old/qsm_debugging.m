...
dir_top = '/data/morrison/wip/lee/nov6/'; % uigedir % must end with /
dir_dicoms_in = strcat(dir_top, 'qsm_dicoms/');
ID1 = extractBefore(dir_dicoms_in,'qsm');
ptid = extractAfter(ID1,'clin/');
ptid = extractBefore(ptid,'_no.consent.yet-addpost');

output_data_path = [dir_top '/processed_QSM/'];
dir_organised_dicoms = strcat(output_data_path, 'raw/');
dir_offset_fixed = strcat(output_data_path, 'offset_fixed/');
...
%Step 2 Correct offset for Re/Im images 
for i= 1:noEchoesExpected
    %GetOffsets(dir_organised_dicoms, i, 'real')
    GetOffsets(dir_organised_dicoms, i, 'imaginary')
end
...
function SortDicoms(input_data_dir, output_data_path, noEchoesExpected)
    % Organises dicoms by echo number
    for i = 1:noEchoesExpected
        mkdir([GetEchoDir(output_data_path,i, 'real')])
        mkdir([GetEchoDir(output_data_path,i, 'imaginary')])
        mkdir([GetEchoDir(output_data_path,i, 'mag')])
    end

    myFiles = dir(fullfile(input_data_dir,'*.dcm')); %gets all dicom files in dir
    parfor k = 1:length(myFiles)
        CopyDicomIfNotFound(string(myFiles(k).name), output_data_path);
    end
end

function CopyDicomIfNotFound(baseFileName, output_data_path)
        fprintf(1, 'Now reading %s\n', baseFileName);
        dicomHeader = dicominfo(baseFileName);
        echoNum = dicomHeader.EchoNumbers;
        copyTo = GetEchoDir(output_data_path, echoNum, GetGREImageType(dicomHeader));
...
function directory = GetEchoDir(output_data_path, echoNo, type)
    % Returns the directory holding dicoms for a specific echo
    % type: provide real/imaginary/mag as a string
    directory = strcat(output_data_path,  'echo', string(echoNo), '/', type, '/');
end


function intensity = GetTopLeftPixel(locFrom)
    % Threadsafe so long as no I/O touching the files at hand
    real1 = dicomread(locFrom);
    intensity = real1(1,1);
end

function GetOffsets(dir_in_top, echoNumber, type)
    ...
    for k=1:length(myFiles)
        ...
        dicomHeader = dicominfo(locFrom);
        % Options"
        % SliceLocation
        x = [x, dicomHeader.InstanceNumber];
        %topLeft = [topLeft, GetTopLeftPixel(locFrom)];
        topLeft = [topLeft, dicomHeader.InStackPositionNumber];

    end



    [xSorted,xSort]=sort(x); %Get the order of B
    topLeft=topLeft(xSort);
    plot(xSorted, topLeft);
