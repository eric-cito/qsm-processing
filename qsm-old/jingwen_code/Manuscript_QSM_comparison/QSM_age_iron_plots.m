...
QSMfile_list = {'QSM_iLSQR' 'QSM_QSIP' 'QSM_SSTGV' 'QSM_SSTV' ...
    ...
    'QSM_QSMGAN' 'QSM_QSMnet' 'QSM_xQSM' 'QSM_iQSM'};
...
% GP, RN, SN, PU, CN % DN, 
% 181 182
BGind = [187 188 210 211 179 180 177 178 189 190]; %  189 190
...
QSM_L = meanStat(:,[181 187 210 179 189 177 208 131],:); % 131
QSM_R = meanStat(:,[181 187 210 179 189 177 208 131]+1,:); % 131
QSM_LR = (QSM_L + QSM_R)/2 - repmat(meanStat(:,REFind,:),[1 8 1]);
...
ax.XTickLabelRotation = 30;
ax.Box = 'off';
ylabel('Corr. Coef. with Iron'); ylim([0.85 1]);
xlim([0 13]);
