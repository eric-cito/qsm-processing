...
DCMout_root = '/working/lupolab/jingwen/001_QSM/02_QSM_HC_age/DICOM_revision2';
mkdir(DCMout_root);
...
% randomly select 20 cases from already evaluated images
Order = randperm(100)';
OldSel = Order > 80;
subjOld = reshape(repmat(subjSel,[1 10])',100,1);
QSMfileOld = repmat(QSMfile_list(1:10)',[10 1]);

subjOld = subjOld(OldSel);
QSMfileOld = QSMfileOld(OldSel);

% combine with 20 new images
subjNew = reshape(repmat(subjSel,[1 2])',20,1);
QSMfileNew = repmat(QSMfile_list(11:12)',[10 1]);
subj = [subjOld; subjNew];
QSMfile = [QSMfileOld; QSMfileNew];

% randomly order the 40 cases
rng(120);
Order = randperm(40)';
...
save([DCMout_root '/../VisualList_revision.mat'], 'T');
...
for ii = 1:length(subj)
    ...
    elseif exist(sprintf('%s/%s/IM%04i.dcm',DCMout_root,[examID '_ax'],1), 'file') == 0 ...
        ...
        meta.PatientName.FamilyName = sprintf('Revision_Case%03i', Order(ii));
