...
% % localField = localField/(b0*gyro*headerAndExtraData.te);
% 
% % noise = 0;
% 
...
% chi = chi .* mask;
% 
% % Jingwen - not the case in FANSI toolbox...
% chi = chi/(b0*gyro*headerAndExtraData.te);

% Jingwen - rewrote main for nlTV

N = size(mask);

kernel = dipole_kernel_fansi(N, headerAndExtraData.voxelSize, 0);

phs_scale = headerAndExtraData.te * gyro * b0 * 2*pi;
 
params = [];
params.input = localField * 2*pi; % HzRad
magn_use = headerAndExtraData.magn(:,:,:,end) .* mask;
...
try algorParam2.qsm.lambda          = algorParam.qsm.lambda;        catch; algorParam2.qsm.lambda           = 1e-4; end
try algorParam2.qsm.mu1             = algorParam.qsm.mu1;           catch; algorParam2.qsm.mu1              = 100*algorParam2.qsm.lambda; end
