function [QSMdata, QSMstats] = ...
    stats_QSM_voxCorr(QSMfile_root, QSMfile_list, ROIfile, Maskfile, ...
    ROIlist, flag_erode, QSMslope, age, medianAge)
...
for nn = 1:length(QSMfile_list)
    ...
    nii = load_nii([QSMfile_root '/MNIreg/' QSMfile_list{nn} '_MNInorm.nii.gz']);
    QSMmap = double(nii.img);
    QSMmap = QSMmap - QSMslope(:,:,:,nn)*(age-medianAge);
