function [ m1, m2, m3 ] = create_freqmasks( spatial_res,kernel )
...
rin = max(spatial_res)*0.5/2; %*0.65/2; % Radial boundaries of the masks are defined as absolute 
rout = max(spatial_res)*0.95/2; %*0.95/2;% frequency values [1/mm]. Please modify if needed.
...
m1 = (m0).*((kernel>0) - (kernel>0.085)); % (1-(abs(kernel)>0.085)); % Emperically found range values
m2 = (m0).*((kernel>0.1) - (kernel>0.2)); % ((abs(kernel)>0.15) - (abs(kernel)>0.3) );
m3 = (m0).*((kernel>0.22) - (kernel>0.32)); % ((abs(kernel)>0.35) - (abs(kernel)>0.6) );
