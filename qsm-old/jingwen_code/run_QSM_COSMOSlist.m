...
for ii = 1:length(subjFolders) 
    ...
    try
        ...
        cd(input_data_path);
        load('sepia_header.mat','header');
        nii = load_untouch_nii([output_data_path '/brain_mask_HD.nii.gz']);
        header.voxelSize = nii.hdr.dime.pixdim(2:4);
        header.matrixSize = nii.hdr.dime.dim(2:4);
        save('sepia_header.mat','header');
        
        %% run
        
%         system(sprintf('rm -r %s/iQSM',output_data_path));
%         delete([output_data_path '/QSM_iQSM_meanEcho.nii.gz']);
        
        QSMfile = dir(output_data_path);
        QSMfile([QSMfile.isdir]) = [];
        QSMfile(cellfun(@isempty,strfind({QSMfile.name},'QSM_'))) = [];
        
        if (opts.All && length(QSMfile) < 13) || (~opts.All && length(QSMfile) < 2)
    ...
    %% plot images
    QSM_file = [output_data_path '/QSM_iQSM_meanEcho.nii.gz'];
