function [Tcomp] = QSM_processing(input_data, output_data_path, opts)
...
%       - opts.QSMGAN           % flag to run, default 0
...
if nargin < 1 || isempty(input_data)
    input_data = struct;
end

%% set default arguments

input_data = check_and_set_default_input(input_data, currentPath);

opts = check_and_set_default_opts(opts);

if exist(input_data.headerFile, 'file') ~= 2
    set_SEPIA_header;
    input_data.headerFile = [pwd '/sepia_header.mat'];
end
load(input_data.headerFile);
...
if exist([output_data_path '/Magni.nii.gz'],'file') ~= 2
    system(sprintf('fslmaths %s %s/Magni.nii.gz -odt float', ...
        input_data.magnitudeFile, output_data_path));
    system(sprintf('fslmaths %s %s/Phase.nii.gz -odt float', ...
        input_data.phaseFile, output_data_path));
end
...
if exist([output_data_path '/brain_mask.nii.gz'],'file') ~= 2
    ...
    cd([output_data_path '/Magni_subvol'])
    for ii = 1:Necho
        MagFile = [output_data_path '/Magni_subvol/Magni000' num2str(ii-1) '.nii.gz'];
        cmd = sprintf('bet2 %s brain000%i -m -n', MagFile, ii-1);
        system(cmd);
        nii = load_untouch_nii([output_data_path '/Magni_subvol/brain000' num2str(ii-1) '_mask.nii.gz']);
        brain_mask(:,:,:,ii) = nii.img;
    end
    brain_mask = double(brain_mask(:,:,:,1)); % double(min(brain_mask,[],4));
...
addpath(genpath([deepMRI_root 'xQSM']));

if opts.All && exist([output_data_path '/QSM_xQSM2_meanEcho.nii.gz'],'file') ~= 2
    ...
    try
        ...
        imSize_xQSM = size(localField_iso);
        if sum(mod(imSize_xQSM, 8)) > 0
            [localField_iso, pos] = ZeroPadding(localField_iso, 8);
        end
        
        if canUseGPU()
            % (1) if your MATLAB is configured with CUDA GPU acceleration
            QSM_recon_xQSM = Eval(localField_iso, 'xQSM_invivo_withNoiseLayer', 'gpu');
        else
            % (2) otherwise if CUDA is not available, use CPU instead, this is much slower
            QSM_recon_xQSM = Eval(localField_iso, 'xQSM_invivo_withNoiseLayer', 'cpu');
        end
        
        % if zeropadding was performed, then do zero-removing before next step;
        if sum(mod(imSize_xQSM, 8)) > 0
            QSM_recon_xQSM = ZeroRemoving(QSM_recon_xQSM, pos);
        end
    ...
    QSM = flip(imresize3(QSM_recon_xQSM,header.matrixSize),2);
    
    % image normalization (mean of brain tissue region set to 0)
    QSM = QSM .* brain_mask;
    QSM = QSM - sum(QSM(:)) / sum(brain_mask(:));
    ...
    nii = load_untouch_nii(nii_mask_path);
    nii.img = QSM_output;
    save_untouch_nii(nii, [output_data_path '/QSM_xQSM2_meanEcho.nii.gz']);
...
deepMRI_root = '/working/lupolab/jingwen/001_QSM/01_Code/deepMRI/';
addpath(genpath([deepMRI_root 'iQSM']));

if opts.All && exist([output_data_path '/QSM_iQSM2_meanEcho.nii.gz'],'file') ~= 2
    
    tic;
    
    try
        run_iQSM(output_data_path, header, TEs);
    catch
        disp('iQSM failed.');
    end
    ...
    % save maps
    nii = load_nii([output_data_path '/brain_mask_HD.nii.gz']);
    brain_mask_ori = double(nii.img);
    nii = load_nii([output_data_path '/iQSM2/iQSM_echo_fitted.nii']);
    QSM = double(nii.img);
    nii.img = QSM.*brain_mask_ori;
    save_nii(nii, [output_data_path '/QSM_iQSM2_meanEcho.nii.gz']);
end
...
try     opts2.QSMGAN    = opts.QSMGAN;      catch;  opts2.QSMGAN    = 0; end
