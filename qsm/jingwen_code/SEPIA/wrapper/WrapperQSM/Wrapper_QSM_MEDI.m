...
TE          = headerAndExtraData.te; % s
CF          = headerAndExtraData.CF;
iMag        = headerAndExtraData.magn(:,:,:,end);
iPha        = headerAndExtraData.phase(:,:,:,end);
...
%% Jingwen modifications according to Eason's code

delta_TE = TE; % second, for single echo it is the same as TE.

iField = iMag .* exp(1i * iPha);

[~, N_std] = Fit_ppm_complex(iField);
...
RDF = RDF*2*pi; % Hz at TE -> rad at TE Jingwen

% Jingwen: in MEDI: x = x/(2*pi*delta_TE*CF)*1e6.*Mask;
