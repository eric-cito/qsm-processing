...
te    = headerAndExtraData.delta_TE; % s
...
% 
localField = localField * 2*pi; % RadHz at te

% with same padsize as QSM_iLSQR
chi = QSM_star(double(localField),double(mask),'H',b0dir,'voxelsize',voxelSize,...
    'padsize',padSize,'TE',te*1e3,'B0',b0);

% chi = QSM_star(localField,mask,'TE',te*1e3,'B0',b0,'H',b0dir,'padsize',padSize,'voxelsize',voxelSize);

% chi = chi; % * te; % Jingwen modified according to STI_template.m
...
try algorParam2.qsm.padSize = algorParam.qsm.padSize; catch; algorParam2.qsm.padSize = [0,0,10]; end % [12,12,12]
