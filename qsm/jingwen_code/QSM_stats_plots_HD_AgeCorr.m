...
indEM = find(strcmp(typeList,'EM')); %  & cagList > 36
...
for rr = 1:3 % 1:length(indList)
    ...
    export_fig(['HD-' indname{rr} '_AgeCorr'], '-png','-transparent'); % close;
end
...
% export_fig('HD-BG_ANOVAPc', '-png','-transparent'); % close;
...
% export_fig('HD-BG_ANOVAPc_compPM', '-png','-transparent'); % close;
...
% export_fig('HD-BG_ANOVAPc_compEM', '-png','-transparent'); % close;
...
% export_fig('HD-BG_ANOVAZc', '-png','-transparent'); % close;
...
export_fig('HD-BG_ANOVAZc_compPM', '-png','-transparent'); % close;
...
export_fig('HD-BG_ANOVAZc_compEM', '-png','-transparent'); % close;
...
export_fig('HD-BG_ES_compPM', '-png','-transparent'); % close;
...
export_fig('HD-BG_ES_compEM', '-png','-transparent'); % close;
...
export_fig('HD-BG_SS_compPM', '-png','-transparent'); % close;

figure('position', [100 100 750 300]);
data = medianROI_SS_BG(:,:,3);
plot_scatter_Z(data, QSMname);
ylabel('Sample Size (HC-EM)'); set(gca, 'YScale', 'log')
export_fig('HD-BG_SS_compEM', '-png','-transparent'); % close;
